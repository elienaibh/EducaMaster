...
import { Menu, X, Home, BookOpen, Brain, LightbulbIcon, Code, Globe, User, Settings, LogOut, Shield } from 'lucide-react';
...
const MainLayout = ({ children }) => {
  ...
  const { currentBoss, healthPercentage } = useBoss();
  ...
  const navItems = [
    ...
    { label: 'Arena do Boss', icon: <Shield className="w-5 h-5" />, path: '/boss', badge: healthPercentage <= 30 ? 'danger' : null },
  ...
  return (
    // src/components/layout/MainLayout/index.jsx (continuação)
   ...
   >
     <div className="flex flex-col h-full">
       ...
       {/* Status do Boss (destaque na barra lateral) */}
       <div className="px-4 py-3 border-b border-neutral-200">
         <div className="flex items-center justify-between mb-1">
           <div className="flex items-center text-neutral-700">
             <Shield className="w-4 h-4 text-danger-500 mr-1" />
             <span className="text-sm font-medium">HP do Boss</span>
           </div>
           <span className="text-xs text-neutral-500">{healthPercentage}%</span>
         </div>
